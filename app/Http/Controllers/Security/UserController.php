	...
	*/
	...
	public function postSave(Request $request)
	{
		...
		if ($validator->fails()) {			
			...
			if(!empty($request->input()['reference']))$userprofile ->reference =  $request->input()['reference'];	
			if(!empty($request->input()['reference_phone']))$userprofile ->reference_phone =  $request->input()['reference_phone'];
			if(!empty($request->input()['reference_adress']))$userprofile ->reference_phone =  $request->input()['reference_adress'];
			
			try {
				$userAffectedRows = User::where('id', Session::get('opaplus.usuario.id'))->update(array('ip' => $user->ip,'name' => $user->name,'email' => $user->email));
				$userProfileAffectedRows = UserProfile::where('user_id', Session::get('opaplus.usuario.id'))->update(array('identificacion' => $userprofile->identificacion,'type_id' => $userprofile->type_id,'names' => $userprofile->names,'surnames' => $userprofile->surnames,'birthdate' => $userprofile->birthdate,'birthplace' => $userprofile->birthplace,'sex' => $userprofile->sex,'adress' => $userprofile->adress,'home' => $userprofile->home,'city' => $userprofile->city,'neighborhood' => $userprofile->neighborhood,'description' => $userprofile->description,'movil_number' => $userprofile->movil_number,'profession' => $userprofile->profession,'paymentadress' => $userprofile->paymentadress,'reference' => $userprofile->reference,'reference_phone' => $userprofile->reference_phone));
			...
			Session::put('opaplus.usuario.type_id', $request->input()['type_id']);
			...
			Session::put('opaplus.usuario.birthplace', $request->input()['birthplace']);
			Session::put('opaplus.usuario.sex', $request->input()['sex']);
			Session::put('opaplus.usuario.adress', $request->input()['adress']);
			//Session::put('opaplus.usuario.home', $request->input()['home']);
			Session::put('opaplus.usuario.city', $request->input()['city']);
			Session::put('opaplus.usuario.neighborhood', $request->input()['neighborhood']);
			...
			//Session::put('opaplus.usuario.profession', $request->input()['profession']);
			//Session::put('opaplus.usuario.paymentadress', $request->input()['paymentadress']);
			if(!empty($request->input()['reference']))Session::put('opaplus.usuario.reference', $request->input()['reference']);
			if(!empty($request->input()['reference_adress']))Session::put('opaplus.usuario.reference_adress', $request->input()['reference_adress']);
			if(!empty($request->input()['reference_phone']))Session::put('opaplus.usuario.reference_phone', $request->input()['reference_phone']);
