		...
		if ($validator->fails()) {
			...
			if (!$this->auth->check()){
				...
				if (!$this->auth->attempt($userdata)){
					...
					$usuario = $user->where('email',Input::get('usuario'))->get()->toArray();
					if(count($usuario)){
						//es un email	
						$userdata = array(
							'name'  => $usuario[0]['name'],
				...
				$array['usuario']['code_adviser']=$result['code_adviser'];
				$array['usuario']['zone']=$result['zone'];
				$array['usuario']['profession']=$result['profession'];
				$array['usuario']['paymentadress']=$result['paymentadress'];
				$array['usuario']['reference']=$result['reference'];	
				$array['usuario']['reference_adress']=$result['reference_adress'];
				$array['usuario']['reference_phone']=$result['reference_phone'];	
				...
				//solo para asesores financiaros, asignamos su ciudades
				$array['usuario']['ciudades'] = "";
				if(!empty($result['zone'])){
					$ciudades_id = explode(",", $result['zone']);
					...
					$array['usuario']['ciudades']=implode(",",$ciudades);
				}
